# Backend Patterns & Best Practices

## Background Tasks

When implementing long-running or periodic background tasks (e.g. importers, cleanup jobs), follow these patterns to ensure robustness and testability.

### Avoid Busy Loops
**Do not use** `select` with a `default` branch for periodic tasks. If logic within the loop inadvertently skips a delay (e.g. via `continue`), it can cause a tight busy loop that consumes 100% CPU.

### Use `time.Timer` for Scheduling
Instead, use an explicit `time.Timer`:
1. Initialize the timer (e.g. to fire immediately or after a delay).
2. Wait on `timer.C`, context cancellation, or other triggers (e.g. forced run channels).
3. Perform the work.
4. Reset the timer with the next delay.

**Example:**
```go
timer := time.NewTimer(0)
defer timer.Stop()

for {
    select {
    case <-ctx.Done():
        return
    case <-forcedRun:
        if !timer.Stop() {
            select {
            case <-timer.C:
            default:
            }
        }
    case <-timer.C:
    }

    // Do work...
    timer.Reset(nextDelay)
}
```

## Update Operations

### Generic Update Helper (API Layer)
Use the `updateEntity` helper in `backend/pkg/server/api/helpers.go` to handle the standard update flow (UserID extraction, DB update call, and error mapping). This reduces boilerplate in API handlers.

### Generic Update Helper (Storage Layer)
Use the `performUpdate` helper in `backend/pkg/database/storage_common.go` for standard CRUD updates. It handles the `UUID parsing -> Record fetching -> Model conversion -> Save -> Audit Log -> Result conversion` sequence generically.

### Audit Logging
When performing Create, Update, or Delete operations in the storage layer, you **MUST** record an audit log using `s.recordAuditLog`.
- **Signature**: `recordAuditLog(tx, userID, entityType, entityID, action, before, after)`
- **Before/After**:
  - **Create**: `before` is `nil`, `after` is the new entity.
  - **Update**: `before` is the entity *before* modification, `after` is the entity *after* modification.
  - **Delete**: `before` is the entity *before* deletion, `after` is `nil`.
- **Note**: The `performUpdate` helper automatically handles this for updates. For Create/Delete, you must call `recordAuditLog` manually.

## Testing

### Mocking Storage
To test background tasks without relying on actual database timing or complex setup:
1. Create a `MockStorage` struct that embeds `database.Storage`.
2. Override only the methods needed for the test.
3. Use channels to signal when methods are called, allowing synchronization without `time.Sleep`.

**Example:**
```go
type MockStorage struct {
    database.Storage
    OnFetchData func() (Data, error)
}

func (m *MockStorage) FetchData() (Data, error) {
    if m.OnFetchData != nil {
        return m.OnFetchData()
    }
    return m.Storage.FetchData()
}
```

### Decimal Literals
When writing tests in Go, avoid using untyped float constants (e.g., `100.50`) where a `decimal.Decimal` is expected in a struct literal. This causes compiler errors. Always use `decimal.NewFromFloat(100.50)` or similar.

## OpenAPI Generator Customization

We maintain custom mustache templates for the Go server generator in `backend/pkg/generated/templates/goserver/`. 

### Key Customizations:
- **`decimal.Decimal` support**: Added helpers to parse decimal values from query and form parameters. Triggered by `x-is-decimal: true` in `openapi.yaml`.
- **File Handle Lifecycle**: Removed automatic file closure from router helpers to allow service layer access. Deferred closure is now handled in the controller template.

## File Handling Best Practices

### Multi-Part File Handles
When receiving a file handle (e.g., `*os.File`) that may have been read by upstream handlers (like for sniffing content type), always perform a seek before reading:

```go
if _, err := file.Seek(0, 0); err != nil {
    return err
}
data, err := io.ReadAll(file)
```

### Resource Management
For files generated by OpenAPI (multipart/form-data), the generated controller now handles the `defer file.Close()`. Service implementations should NOT close the file if they don't own it.

## Transaction Update Safety

### Safe vs. Unsafe Updates
The application strictly separates public API updates from internal logic updates for Transactions:
- **Public API**: Use `UpdateTransaction` (Safe). It intentionally **preserves** non-editable fields (`IsAuto`, `ExternalIDs`, `MatcherID`, `UnprocessedSources`, `SuspiciousReasons`, `MergedIntoID`, `MergedAt`, `AutoMatchSkipReason`). This prevents user edits from accidentally corrupting internal state (e.g. user toggling `IsAuto`).
- **Internal Logic**: Use `UpdateTransactionInternal` (Unsafe). It allows updating **ALL** fields. Use this for backend processes like auto-matching, bank import handling, or data migrations that need to modify protected fields.

### Pattern for New Methods
If adding new update methods to `Storage`:
1. Implement a private helper (e.g., `updateTransaction`) that takes a `preserveProtected bool` flag.
2. Expose a public safe method (calls helper with `true`).
3. Expose a public internal method (calls helper with `false`).
