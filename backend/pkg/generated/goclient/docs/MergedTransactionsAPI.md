...
------------- | ------------- | -------------
[**GetMergedTransaction**](MergedTransactionsAPI.md#GetMergedTransaction) | **Get** /v1/mergedTransactions/{id} | get merged transaction details by original transaction ID
...
## GetMergedTransaction

> MergedTransaction GetMergedTransaction(ctx, id).Execute()

get merged transaction details by original transaction ID
...
func main() {
	id := "38400000-8cf0-11bd-b23e-10b96e4ef00d" // string | Original ID of the transaction that was merged

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.MergedTransactionsAPI.GetMergedTransaction(context.Background(), id).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `MergedTransactionsAPI.GetMergedTransaction``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetMergedTransaction`: MergedTransaction
	fmt.Fprintf(os.Stdout, "Response from `MergedTransactionsAPI.GetMergedTransaction`: %v\n", resp)
}
```
...
**id** | **string** | Original ID of the transaction that was merged | 

### Other Parameters

Other parameters are passed through a pointer to a apiGetMergedTransactionRequest struct via the builder pattern
