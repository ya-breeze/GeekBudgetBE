...
var _ = Describe("Transaction Utilities", func() {
	Describe("GetIncreases", func() {
		It("should correctly calculate increases for single currency", func() {
			movements := []goserver.Movement{
				{Amount: decimal.NewFromInt(100), CurrencyId: "USD"},
				{Amount: decimal.NewFromInt(-50), CurrencyId: "USD"},
				{Amount: decimal.NewFromInt(200), CurrencyId: "USD"},
			}
			increases := common.GetIncreases(movements)
			Expect(increases).To(HaveLen(1))
			Expect(increases["USD"]).To(Equal(decimal.NewFromInt(300)))
		})
		...
		It("should correctly calculate increases for multiple currencies", func() {
			movements := []goserver.Movement{
				...
				{Amount: decimal.NewFromInt(-25), CurrencyId: "USD"},
				{Amount: decimal.NewFromInt(150), CurrencyId: "EUR"},
			}
			increases := common.GetIncreases(movements)
			Expect(increases).To(HaveLen(2))
			Expect(increases["USD"]).To(Equal(decimal.NewFromInt(100)))
			Expect(increases["EUR"]).To(Equal(decimal.NewFromInt(200)))
		})
		...
		It("should handle negative net increase by taking absolute of negative sum", func() {
			...
			movements := []goserver.Movement{
				{Amount: decimal.NewFromInt(10), CurrencyId: "USD"},
				{Amount: decimal.NewFromInt(-100), CurrencyId: "USD"},
			}
	...
	Describe("IsDuplicate", func() {
		var (
			now = time.Now()
			m1  = []goserver.Movement{{Amount: decimal.NewFromInt(100), CurrencyId: "USD"}}
			m2  = []goserver.Movement{{Amount: decimal.NewFromInt(100), CurrencyId: "USD"}}
		)
		...
		It("should not match transactions with different amounts", func() {
			mDiff := []goserver.Movement{{Amount: decimal.NewFromInt(101), CurrencyId: "USD"}}
			Expect(common.IsDuplicate(now, m1, now, mDiff)).To(BeFalse())
		})

		It("should not match transactions with different currencies", func() {
			mDiff := []goserver.Movement{{Amount: decimal.NewFromInt(100), CurrencyId: "EUR"}}
			Expect(common.IsDuplicate(now, m1, now, mDiff)).To(BeFalse())
		})
		...
		It("should match multi-movement transactions with same total increases", func() {
			ma := []goserver.Movement{
				{Amount: decimal.NewFromInt(100), CurrencyId: "USD"},
				{Amount: decimal.NewFromInt(-100), CurrencyId: "USD"}, // should be ignored for increase calculation
				{Amount: decimal.NewFromInt(50), CurrencyId: "EUR"},
			}
