...
The hero section SHALL use a two-column layout with the app tagline on the left and the login form on the right, collapsing to a single column on mobile screens (≤768px).

#### Scenario: Desktop hero layout
- **WHEN** the landing page is viewed on a screen wider than 768px
- **THEN** the hero shows tagline/subtitle on the left and login form on the right, side by side

#### Scenario: Mobile hero layout
- **WHEN** the landing page is viewed on a screen 768px wide or narrower
- **THEN** the hero stacks to a single column with tagline above and login form below
